...
import com.limkai.parentcontrol3.util.SortEnum;
...
public class DataManager {
    ...
    public List<AppItem> getApps(Context context, int sort, int offset, String relationshipId) {
...
//need to set to true
        ...
        // upload to firebase
        if(newList.size() >0){
            DatabaseReference ref = FirebaseDatabase.getInstance().getReference("AppItem/" + relationshipId);
            ref.removeValue();
            for(AppItem A : newList){
                ref.push().setValue(A);
            }
        }
        return newList;
